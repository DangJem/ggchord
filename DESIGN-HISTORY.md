...
本文只保留 v0.11.0 及之前的实现审计、迁移和发布验收，不代表当前接口。
历史段落中出现的函数和默认值按当时版本原样记录；其中部分接口后来已经删除。
当前契约、长期开发规则及未来规划统一见
[DESIGN-ROADMAP.md](DESIGN-ROADMAP.md)，具体用户迁移以 NEWS 为准。
...
灵活参数格式的现行保留边界已迁至 `DESIGN-ROADMAP.md`。本档案中的旧参数名仅用于
解释对应版本的历史迁移，不构成当前使用建议。
