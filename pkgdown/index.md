...
`ggchord` is an R package built on `ggplot2` for drawing **chord diagrams of
multi-sequence data** using the layered grammar of graphics. You provide three
tidy data frames (`seq_data`, `ribbon_data`, `gene_data`) and stack
`geom_*` layers like any other ggplot2 plot.
