...
A ggchord plot can be converted to an interactive chart with
[`plotly::ggplotly()`](https://rdrr.io/pkg/plotly/man/ggplotly.html)
(requires the `plotly` package). Ribbons, gene arrows, axes, and the Seq
ID / Strand / Identity legends are all preserved:
...
> **Parameter formats**: sequence-level parameters (`seq_radius`,
> `seq_gap`, etc.) accept a single value, an unnamed vector, or a named
> vector. Gene-level parameters additionally accept per-strand (`+`/`-`)
> list formats,
> e.g. `gene_offset = list(c("+" = 0.2, "-" = -0.2), ...)`.
