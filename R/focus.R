...
ggchord_focus_validate_loci <- function(seq_data, loci, expand, caller) {
  ...
  if (!is.data.frame(loci) ||
      !all(c("seq_id", "start", "end") %in% names(loci))) {
    ggchord_stop(caller, ": loci must contain seq_id, start and end")
  }
  ...
  if ("locus_id" %in% names(out)) {
    ...
    repeated <- duplicated(out$seq_id) | duplicated(out$seq_id, fromLast = TRUE)
    output_id <- out$seq_id
    output_id[repeated] <- paste0(
      out$seq_id[repeated], ":", out$.focus_start[repeated], "-",
...
#'
#' @param seq_data Sequence table containing `seq_id` and `length`.
#' @param ribbon_data Optional ggchord alignment table.
#' @param gene_data Optional gene or feature table.
#' @param loci Data frame containing `seq_id`, `start`, and `end`, and
#'   optionally `locus_id`.
...
focus_ggchord_data <- function(
  ...
  if (!is.null(gene_data)) {
    ...
    if (!is.data.frame(gene_data) || any(!required %in% names(gene_data)) ||
      ...
      ggchord_stop(caller, ": gene_data must contain seq_id, numeric start/end, and strand")
    }
    pieces <- list()
    for (i in seq_len(nrow(gene_data))) {
      hits <- which(focus$.source_seq_id == as.character(gene_data$seq_id[i]))
      if (!length(hits)) next
      a <- gene_data$start[i]
      b <- gene_data$end[i]
      glo <- min(a, b)
      ...
      for (j in hits) {
        ...
        row <- as.data.frame(gene_data[i, , drop = FALSE],
                             stringsAsFactors = FALSE)
        row$seq_id <- focus$.output_seq_id[j]
        if (a <= b) {
        ...
        row$.source_seq_id <- focus$.source_seq_id[j]
        row$.source_row <- i
    ...
    gene_out <- if (length(pieces)) ggchord_rbind_fill(pieces) else {
      ...
      empty$.source_seq_id <- character(0)
  ...
  if (!is.null(ribbon_data)) {
    ...
    for (i in seq_len(nrow(ribbon_data))) {
      qhits <- which(focus$.source_seq_id == as.character(ribbon_data$qaccver[i]))
      shits <- which(focus$.source_seq_id == as.character(ribbon_data$saccver[i]))
      ...
      for (qj in qhits) for (sj in shits) {
        ...
        row$qaccver <- focus$.output_seq_id[qj]
        row$saccver <- focus$.output_seq_id[sj]
        ...
        row$length <- max(1, round(ribbon_data$length[i] * diff(tr)))
        row$.source_row <- i
