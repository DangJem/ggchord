...
- 通用 `feature_shape` 扩展为 `arrow`、`compact_arrow`、
  ...
  `lollipop`；promoter、primer 和极短 feature 使用独立的轻量 geometry，
  长箭头的头部由显示长度限制，不随生物学区间同比例增长；
- `find_common_features()` 输出规范化 `feature_class`、语义 shape 和
  `preferred_lane`。这些列只提供缺省提示；用户显式 shape、fill、linewidth、
  Position 与手动 scale 始终优先；
- feature lane allocator 同时考虑跨原点区间、实际 glyph 宽度、描边和最小径向
  间隙；语义 lane 发生冲突时仍按确定性的 interval coloring 就近改派，不生成
  无意义的空轨道；
...
- `geom_feature_label()`、`geom_feature_label_repel()` 复用 gene label 的文本测量、
  ...
  lineheight 和设备实测 bbox，依次选择 `inside`、`adjacent` 或 `callout`，并在
  layout export 中公开 `feature_label_mode`；
- feature 标签使用统一的切向正置规则；放不下时先沿局部切线按基因组顺序错层，
  再搜索相邻径向 lane。求解器同时避开已放标签、实际 feature polygon、圆心保留区
  和 backbone 边界，只对明显移位的 adjacent/callout 文字绘制裁切到真实 bbox 的
  细灰短 connector；
