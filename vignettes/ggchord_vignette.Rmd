...
## 4c. Ribbon mapping and direction

`geom_ribbon()` can map any numeric column to a continuous fill (not only
`pident`), map alpha from a numeric column, and distinguish same- vs
...
`ribbon_outline_by` / `ribbon_linetype_by` map outline colour or linetype from
a discrete column.

## 4d. Highlights and generic features

Mark sequence intervals with `geom_seq_region()` and emphasise selected ribbons
with `geom_ribbon_highlight()`:
...
`geom_feature()` is a generic convenience wrapper around `geom_gene()` for CDS,
tRNA, rRNA, repeat, CRISPR, promoter or user-defined features:
...
| `ribbon_color_by` | character | NULL | Map a numeric column to a continuous fill instead of `pident` |
| `ribbon_alpha_by` | character | NULL | Map a numeric column to ribbon alpha |
| `ribbon_outline_by` | character | NULL | Map a discrete column to outline colour |
| `ribbon_linetype_by` | character | NULL | Map a discrete column to outline linetype |
| `ribbon_direction` | character | "none" | Distinguish same/reverse alignments: `"none"`, `"alpha"`, `"outline"` or `"linetype"` |
...
## 10. Quick function reference

One minimal runnable call for every exported function. The calls below build
the plot without opening a graphics device, so this reference stays short.
