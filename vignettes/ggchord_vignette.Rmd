...
A basic pUC19 map needs only the circular backbone and centre metadata:
...
`find_common_features()` supplies semantic classes, glyph hints, and preferred
lanes for its built-in annotations. Long genes and origins remain shouldered
arrows; promoters and primers use thinner dedicated arrows; binding sites,
operators, and MCS regions use compact blocks. These are defaults rather than
name-based rules, so explicit aesthetics and manual scales always win.

With the default `label_layout = "feature"`, text is measured with its final
font settings and tried inside the usable feature body first. Labels that do
not fit are placed adjacent to the glyph or in a short callout. Dense clusters
stagger mainly along the local tangent, then across radial lanes, while avoiding
other feature polygons and label boxes. The exported label layout records the
resolved `feature_label_mode` (`inside`, `adjacent`, or `callout`). Restriction
labels are deliberately solved by their separate layer.

The dense pBluescript II SK(+) multiple-cloning region is a restriction-label
stress test. Real sites retain their anchors. Every enzyme-facing label edge
follows one close circular contour in genomic order. Sparse labels retain their
natural angles, while dense lateral clusters use a compact ordered fan with
uniform measured spacing. Each dense site keeps
...
The parent enzyme catalogue is part of the display definition, not part of
sequence matching. The explicit list below reproduces the supplied reference
map. For a database-derived general-purpose view, search all patterns and use
`parent_set = "commercial_nonredundant"` in `filter_restriction_sites()`.
Coordinates can still expose database-version differences: REBASE 609 defines
NmeAIII as `GCCGAG(21/19)`, giving 2195 here, whereas the supplied older
reference image labels that cut as 2194.
