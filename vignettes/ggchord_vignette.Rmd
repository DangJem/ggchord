...
## Single-genome circles and restriction sites

Use `coord_genome()` when the input describes one complete genome. A zero
opening closes the circle; `gap` opens it by a number of degrees. The
coordinate also owns the origin rotation and genomic direction.
...
recognition and cut coordinates, overhang type, strand, and a traceable enzyme
definition source. Supply a definition data frame for custom IUPAC motifs.
...
`link_avoid` defaults to `none`; opt into `smooth` or `uniform` when needed,
and explicit gaps win. `link_branch` defaults to `none`, with optional `query` or `subject` trunks
