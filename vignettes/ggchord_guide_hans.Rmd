---
title: "ggchord 中文使用指南"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ggchord 中文使用指南}
...
可通过 `seq_group`（`seq_data` 中的列名，或命名向量）对序列分组。不同组之间会插入更大空隙，并可自定义组标签：
...
`geom_seq(seq_group_labels = FALSE)` 保留组间空隙但隐藏组标签；`seq_group_gap`
控制组间额外空隙大小。

## 4c. Ribbon 映射与方向

`geom_ribbon()` 可将任意数值列映射为连续填充色（不再局限于 `pident`），也可将数值列映射为透明度，并区分同向/反向比对：
...
`ribbon_outline_by` / `ribbon_linetype_by` 可将离散列的取值映射为描边颜色或线型。

## 4d. 高亮与通用 feature

使用 `geom_seq_region()` 标记序列区间，使用 `geom_ribbon_highlight()` 强调选中的连接带：
...
`geom_feature()` 是 `geom_gene()` 的通用便捷封装，可用于 CDS、tRNA、rRNA、repeat、CRISPR、promoter 或自定义 feature：
...
```{r seq-default-img, echo=FALSE, fig.cap="默认参数下的序列弦图。"}
...
```{r ribbon-pident-img, echo=FALSE, fig.cap="按相似度着色的连接带。"}
...
当注释较长或基因较密集时，标签容易重叠。`geom_gene_label_repel()` 采用类似 ggrepel 的力导向布局将标签推开，并用引导线连接对应的基因：
...
```{r gene-repel-img, echo=FALSE, fig.cap="带引导线的防重叠基因标签。"}
...
坐标轴用主/次刻度标注序列位置。`geom_seq_label()` 在弧线周围放置序列名称：
...
```{r axis-label-img, echo=FALSE, fig.cap="坐标轴与序列标签。"}
...
**图例摆放**。默认情况下各图层的图例独立摆放。若将 `legend_position` 设为 `NULL`，该图例将遵循 `theme(legend.position = ...)`，从而可以把所有图例放在一起：
...
```{r legend-bottom-img, echo=FALSE, fig.cap="通过 theme() 将所有图例放在底部。"}
...
```{r combined-fine-img, echo=FALSE, fig.cap="精细控制下的完整弦图。"}
...
| `seq_group` | NULL / 列名 / 向量 | NULL | 每条序列的分组 |
| `seq_group_gap` | 数值 | 0.08 | 组间额外空隙比例 |
| `seq_group_labels` | 逻辑 / 字符 | TRUE | 是否绘制组标签；命名向量可覆盖标签文字 |
| `seq_group_label_radius` | 数值 | 1.35 | 组标签径向位置（`1` = 在弧线上，`> 1` = 外侧） |
| `seq_group_colors` | 命名颜色向量 | grey20 | 组标签颜色 |
...
| `ribbon_color_by` | 字符 | NULL | 将数值列映射为连续填充色，替代 `pident` |
| `ribbon_alpha_by` | 字符 | NULL | 将数值列映射为 ribbon 透明度 |
| `ribbon_outline_by` | 字符 | NULL | 将离散列映射为描边颜色 |
| `ribbon_linetype_by` | 字符 | NULL | 将离散列映射为描边线型 |
| `ribbon_direction` | 字符 | "none" | 区分同向/反向比对：`"none"`、`"alpha"`、`"outline"` 或 `"linetype"` |
...
每个导出函数一条最小可运行示例。以下调用只构建图形、不打开绘图设备，因此速查表保持精简。

```{r quick-ref-hans}
# ---- 构建图形 ----
p <- ggchord(seq_data_example, ribbon_data_example, gene_data_example)
invisible(coord_chord())
invisible(ggplot2::ggplot_build(p + geom_seq() + geom_ribbon()))
invisible(ggplot2::ggplot_build(p)); invisible(get_chord_layout())

# ---- 图层 ----
invisible(ggplot2::ggplot_build(ggchord(seq_data_example) + geom_seq()))
invisible(ggplot2::ggplot_build(ggchord(seq_data_example, ribbon_data_example) +
  geom_seq() + geom_ribbon()))
invisible(ggplot2::ggplot_build(ggchord(seq_data_example, gene_data = gene_data_example) +
  geom_seq() + geom_gene()))
invisible(ggplot2::ggplot_build(ggchord(seq_data_example, gene_data = gene_data_example) +
  geom_seq() + geom_gene() + geom_gene_label()))
invisible(ggplot2::ggplot_build(ggchord(seq_data_example, gene_data = gene_data_example) +
  geom_seq() + geom_gene() + geom_gene_label_repel()))
invisible(ggplot2::ggplot_build(ggchord(seq_data_example) + geom_seq() + geom_axis()))
invisible(ggplot2::ggplot_build(ggchord(seq_data_example) + geom_seq() + geom_seq_label()))

regions <- data.frame(seq_id = "MT108731.1", start = 1000, end = 4000,
                      color = "orange")
invisible(ggplot2::ggplot_build(ggchord(seq_data_example) + geom_seq() +
  geom_seq_region(regions)))

invisible(ggplot2::ggplot_build(ggchord(seq_data_example, ribbon_data_example) +
  geom_seq() + geom_ribbon() + geom_ribbon_highlight(ribbon_ids = 1)))

features <- data.frame(seq_id = "MT108731.1", start = 1000, end = 4000,
                       strand = "+", type = "CDS")
invisible(ggplot2::ggplot_build(ggchord(seq_data_example) + geom_seq() +
  geom_feature(features)))

# ---- 数据导入 ----
invisible(read_fasta_lengths(files = "examples/fasta/*.fna"))
invisible(read_blast(files = "examples/blastn/*.o7"))
invisible(read_gff3(files = "examples/gff3/*.gff3"))

# ---- 校验与清理 ----
invisible(validate_ggchord_data(seq_data_example, ribbon_data_example, gene_data_example))
invisible(clean_ggchord_data(seq_data_example, ribbon_data_example, gene_data_example))

# ---- Ribbon 处理 ----
invisible(filter_ggchord_ribbons(ribbon_data_example, min_pident = 90))
dedup <- deduplicate_ggchord_ribbons(ribbon_data_example, by = "exact")
invisible(merge_ggchord_ribbons(dedup$data))
```
