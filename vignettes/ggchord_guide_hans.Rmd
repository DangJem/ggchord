...
data(plasmid_sequence_example)
puc <- subset(plasmid_sequence_example, label == "pUC19c")
ggchord(puc) +
...
data(plasmid_sequence_example)
data(plasmid_feature_example)
data(plasmid_restriction_example)
seq <- subset(plasmid_sequence_example, label == "pBR322")
features <- subset(plasmid_feature_example, accver == seq$accver)
genes <- subset(features, type == "CDS")
other <- subset(features, type != "CDS")
sites <- subset(plasmid_restriction_example, accver == seq$accver) |>
  filter_restriction_sites(set = "unique_6plus")

ggchord(seq, gene_data = genes) +
  geom_seq(seq_style = "double") +
  geom_gene(position = "plasmid") +
  geom_feature(aes(feature_fill = type, feature_shape = type),
    data = other, position = "plasmid") +
  geom_feature_label_repel(aes(label = anno), data = features,
    position = "plasmid", label_layout = "callout") +
  geom_restriction_site(data = sites, leader = "trunk") +
  geom_seq_center_label() +
...
anchor 保持不动，共享外侧 trunk 以确定性方式分配标签位置：

```{r pbluescript-restriction-map, eval=FALSE}
blue <- subset(plasmid_sequence_example, label == "pBluescript II SK(+)")
blue_sites <- subset(plasmid_restriction_example, accver == blue$accver)
ggchord(blue) +
  geom_seq(seq_style = "double") +
  geom_restriction_site(data = blue_sites, leader = "trunk") +
  geom_seq_center_label() +
  scale_seq_position_continuous() +
  coord_circular(rotation = 90) +
  theme_ggchord_plasmid()
