...
输入是一条完整基因组时，使用 `coord_genome()`。`gap = 0` 形成闭合圆，正数按
角度打开缺口；原点旋转和基因组方向也由这个坐标入口管理。

```{r single-genome, eval=FALSE}
data(single_genome_example)
data(single_gene_example)
data(restriction_site_example)

ggchord(single_genome_example, gene_data = single_gene_example) +
  geom_seq() +
  geom_gene() +
...
可追踪的酶定义来源；自定义 IUPAC motif 可通过定义表传入。

```{r restriction-search, eval=FALSE}
sites <- find_restriction_sites(
  c(plasmid = "AAAAGAATTCTTTGGATCC"),
...
`link_avoid` 默认为 `none`，可手动设为 `smooth` 或 `uniform`；显式 gap 优先。
