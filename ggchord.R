...
#' @param gene_offset 数值或向量，基因箭头与序列圆弧之间的径向偏移距离，默认0.03
...
ggchord <- function(
  ...
  geneGap         <- process_sequence_param(gene_offset, seqs, "gene_offset", default_value = 0.03)  # 处理基因间隙参数
...
if (nrow(gene_track) > 0) {
  ...
  if (nrow(valid_genes) > 0) {
    gene_arrows <- do.call(rbind, lapply(1:nrow(valid_genes), function(i) {
      ...
      # 确定箭头的半径（在序列线外侧或内侧）
      arrow_radius <- seqRadius[seq_id] + geneGap[seq_id]
