...
test_that("local entity clearance keeps unobstructed fronts close", {
  ...
  expect_equal(default$obstacles, uniform$obstacles)
  expect_equal(default$ribbon_polys$q_gap, uniform$ribbon_polys$q_gap)
  expect_equal(default$ribbon_polys$s_gap, uniform$ribbon_polys$s_gap)
  expect_gt(nrow(default$obstacles), 0L)
